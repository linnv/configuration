...
func ConstDemo() {
	...
	logx.Debugf("UrlIndex: %+v\n", UrlIndex)
	logx.Debugf("WidthIndex: %+v\n", WidthIndex)
	logx.Debugf("HeightIndex: %+v\n", HeightIndex)
	logx.Debugf("TrackIndex: %+v\n", TrackIndex)
	logx.Debugf("ClickIndex: %+v\n", ClickIndex)

	var ls = []string{
		"one",
	}
	logx.Debugf("ls[UrlIndex]: %+v\n", ls[UrlIndex])
...
one:
	for i := 0; i < 10000; i++ {
		...
		j++
		if j%2 == 0 {
			//if use break without label,it only break the nearest loop
			// pass = true
			println("i ", i, " j ", j)
			continue one
		}
