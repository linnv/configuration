...
func OneDemo() {
	println("//<<-------------------------OneDemo start-----------")
	start := time.Now()
	defer func() {
		recover()
		println("got it\n")
		panic("do it again, this msg will be deliver to outter function")
	}()
	panic("from one")
	fmt.Printf("OneDemo costs  %d millisecons actually %v\n", time.Since(start).Nanoseconds()/1000000, time.Since(start))
	println("//---------------------------OneDemo end----------->>")
}
...
func JustDemo() {
	...
	//getOne implements ...
	getOne := func() error {
		return errors.Trace(errDemo("one"))
	}

	r := getOne()
	s := errors.ErrorStack(r)
	log.Printf("s: %+v\n", s)
	println("-----------------------------JustDemo end>>>")
	return
}

func errDemo(str string) error {
	println("//<<-------------------------errDemo start-----------")
	start := time.Now()

	fmt.Printf("errDemo costs  %d millisecons actually %v\n", time.Since(start).Nanoseconds()/1000000, time.Since(start))
	println("//---------------------------errDemo end----------->>")
	return errors.Trace(fmt.Errorf("%s", str))
}

func RetDeferDemo(i int) (j int) {
	println("//<<------------------------- RetDeferDemo start-----------")
	start := time.Now()
	...
	j = 1
	fmt.Printf(" RetDeferDemo costs  %d millisecons actually %v\n", time.Since(start).Nanoseconds()/1000000, time.Since(start))
	println("//--------------------------- RetDeferDemo end----------->>")
	return 9
}
