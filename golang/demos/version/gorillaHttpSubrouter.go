...
func main() {
	...
	r := s.PathPrefix("/html/").Subrouter() //前缀路由 r.PathPrefix("/products/") 匹配所有 “/products/” 开头的 Request
	r.HandleFunc("/", rootHandler)
	r.HandleFunc("/view/", viewHandler)
	// r.HandleFunc("/html/", htmlHandler) ///url/html/ works,but /url/html doesn't work
	// r.HandleFunc("/demo", htmlHandler) ///url/html/ works,but /url/html doesn't work     /url/html and /url/html/ is different
	r.HandleFunc("/blog.html", htmlHandler) ///url/html/ works,but /url/html doesn't work     /url/html and /url/html/ is different
	http.Handle("/", r)
