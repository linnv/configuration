...
func UniRandDemo() {
	...
	slowMove := is[0]
	fastMove := is[is[0]-1] - 1
	c := 0
	for {
		c++
		slowMove = is[slowMove] - 1
		fastMove = is[is[fastMove]-1] - 1
		log.Printf("slowMove: %+v\n", slowMove)
		log.Printf("fastMove: %+v\n", fastMove)
	...
	for {
		...
		slowMove = is[slowMove]
		fastMove = is[fastMove]
		if slowMove == fastMove {
			break
		}
	}
	log.Printf("slowMove: %+v r\n", slowMove)
	fmt.Printf("UniRandDemo costs  %d millisecons actually %v\n", time.Since(start).Nanoseconds()/1000000, time.Since(start))
	println("//---------------------------UniRandDemo end----------->>")
}
